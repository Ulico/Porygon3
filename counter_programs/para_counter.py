...
for link in links:
    ...
    num_freezes_p1 = len(re.findall(r"-status\|p1[ab]: .*\|par", data))
    num_freezes_p2 = len(re.findall(r"-status\|p2[ab]: .*\|par", data))
...
plt.title(f"SBL Season 7 Paras Count ({'Given' if mode == 0 else 'Received'})")
