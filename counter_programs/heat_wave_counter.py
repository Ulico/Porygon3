...
for link in links:
    ...
    num_crits_p1 = len(re.findall(r"\|move\|p1.*?\|Heat Wave\|.*$\n\|-miss\|", data, re.MULTILINE))
    num_crits_p2 = len(re.findall(r"\|move\|p2.*?\|Heat Wave\|.*$\n\|-miss\|", data, re.MULTILINE))
...
sorted_data = {
    k: v for k, v in sorted(crit_dict.items(), key=lambda item: item[1], reverse=False) if v >0
}
...
plt.title(f"SBL Season Heat Wave 9 Miss Count ({'Given' if mode == 0 else 'Received'})")
