...
for link in links:
    ...
    # if 'par' in data and '☆RobbyRabs' in data:
    #     print(data)
    # print(player1, player2, num_freezes_p1, num_freezes_p2)

    num_freezes_p1 = len(re.findall(r"cant\|p1[ab]: .*\|flinch", data))
    num_freezes_p2 = len(re.findall(r"cant\|p2[ab]: .*\|flinch", data))
    ...
    if mode == 0:
        ...
        freeze_dict[player1] = freeze_dict.get(player1, 0) + num_freezes_p1
        freeze_dict[player2] = freeze_dict.get(player2, 0) + num_freezes_p2
...
plt.title(f"SBL Season 9 Flinch Count ({'Given' if mode == 0 else 'Received'})")
